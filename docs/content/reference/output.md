...
describes what users get. Pick a format with `-o`, or let hn2 choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
hn2 <command> -o table   # aligned columns for reading
hn2 <command> -o jsonl   # one JSON object per line, for piping
hn2 <command> -o json    # a single JSON array
hn2 <command> -o csv     # spreadsheet friendly
hn2 <command> -o tsv     # tab-separated
hn2 <command> -o url     # just the URL column
hn2 <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
hn2 <command> --fields id,title,url
```
...
```bash
hn2 <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
hn2 <command>            # a table, because this is a terminal
hn2 <command> | wc -l    # JSONL, because this is a pipe
```
