...
import (
	...
	"github.com/tamnd/hackernoon-cli/pkg/render"
)
...
type App struct {
	client *hackernoon.Client
	cfg    hackernoon.Config
	out    io.Writer
...
func Root() *cobra.Command {
	...
	root := &cobra.Command{
		...
		Long: `hn2 reads Hackernoon via public RSS feeds — no API key required.
It returns records as table, JSON, JSONL, CSV, TSV, or URLs.

hn2 is an independent tool and is not affiliated with Hackernoon.`,
		SilenceUsage:  true,
		SilenceErrors: true,
	...
	pf.StringVar(&app.cfg.BaseURL, "base-url", app.cfg.BaseURL, "Hackernoon base URL")
...
func (a *App) setup() error {
	...
	a.client = hackernoon.NewClient(a.cfg)
	return nil
}

func (a *App) render(records any) error {
	r := render.New(a.out, render.Format(a.output), a.fields, a.noHeader, a.template)
